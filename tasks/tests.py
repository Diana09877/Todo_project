...
class TaskTests(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(email='user@example.com', first_name='First', last_name='Last', password='pass123')
        self.client.login(email='user@example.com', password='pass123')

    def test_create_task(self):
        response = self.client.post(reverse('task_list'), data={'title': 'New Task'})
